...
void setup() {
  pinMode(18, OUTPUT);
  Serial.begin(115200);
  bdmSerial.begin(57600);  // ESP32_TX2/RX2 = G17/G16
  digitalWrite(18, LOW);
  delay(100);
...
void loop() {
  if (bdmSerial.available() > 0) {
    ...
    if ((myReadBuffer[0] == SYNC) && (myReadBuffer[1] == SYNC)) {  //find data header
      byte pLength = myReadBuffer[2];
      if (pLength == 0x4) dataType = 0;   //dataType is adc val.
      if (pLength == 0x12) dataType = 1;  //dataType is quality & HR.
    }

    for (byte i = 0; i < 19; i++) {
      myReadBuffer[i] = 0;  //clear my serial buffer.
    }
